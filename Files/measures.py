...
def compute_pre_rec(gt,mask,mybins=np.arange(0,256)):
    ...
    #for i in range(0,256):
      #avg_TP = np.mean(pp_hist_flip_cum)
    #for i in range(0,256):
     # avg_FP = np.mean(nn_hist_flip_cum)
    #for i in range(0,256):
      #avg_FN = np.mean(gtNum)
    #for i in range(0,256):
    ...
    # print("TN:{}".format(1-(pp_hist_flip_cum+nn_hist_flip_cum+gtNum)))

    #print('Precision:{}'.format(avg_prec))
...
#       F-measure (beta) 'FM': numpy array with shape of (num_rs_dir, 256)
    ...
    FM = 2*PRE*REC/(PRE+REC) # num_rs_dirx255
